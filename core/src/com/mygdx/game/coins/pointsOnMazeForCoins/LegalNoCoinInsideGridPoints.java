...
public class LegalNoCoinInsideGridPoints {
    public static float[][] returnLegalButNoCoinPoints()
    {
        float[][] allNocoinLegalInsideGridPoints = new float[30][32];
        for(int i=0;i<30;i++){
            for(int j=0;j<32;j++)allNocoinLegalInsideGridPoints[i][j]=0;
        }

        int y=12;
        ArrayList<Integer> xAxis = new ArrayList<>(Arrays. asList(10,19));
        for(int i=0;i<xAxis.size();i++) allNocoinLegalInsideGridPoints[xAxis.get(i)][y]=1;
        xAxis.clear();
        ...
        return allNocoinLegalInsideGridPoints;
