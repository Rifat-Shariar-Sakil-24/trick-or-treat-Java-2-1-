package com.mygdx.game.movementActivityOfPlayerAndMonsters.monstersMovementActivity.decidingActivityOnOrOffForRedMonster.redMonsterMovementOn.redMonsterMovementBasedOnModes.redMonsterScatterMode;

import com.mygdx.game.TheLastLife;
import com.mygdx.game.drawing.monsters.redMonster.redNormalDrawing.RedDrawNormal;

public class RedMonsterScatterMode extends TheLastLife {
    public RedMonsterScatterMode()
    {
    ...
    private void redAlgorithm() {
        ...
        if(On==false)
        {
            ...
            {
                float X = (redGhostX+redGhostMove-redGhostScatterX)* (redGhostX+redGhostMove-redGhostScatterX);
                float Y = (redGhostY-redGhostScatterY) * (redGhostY-redGhostScatterY);
            ...
            {
                float X = (redGhostX-redGhostScatterX)* (redGhostX-redGhostScatterX);
                float Y = (redGhostY+redGhostMove-redGhostScatterY) * (redGhostY+redGhostMove-redGhostScatterY);
                float distanceXYTrial = (float) Math.sqrt(X+Y);
                if(distanceXYTrial < distanceXY)
                {
                    ind = 3;
